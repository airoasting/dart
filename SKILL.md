...
> **웹 검색도 한 번에 병렬로.** 뉴스·부문 매출·주가 앵커·애널리스트 목표가 검색을 하나씩 순차로 돌리지 말고, 한 메시지에 여러 `WebSearch`/`WebFetch`를 몰아 동시에 보낸다.
...
|---|---|---|
| **A. 웹 리서치** | 기업명·종목코드·분기·발표일 | `NEWS`(6~8), `ANALYSTS`(≥3), `TP_BARS`, `CONS`, 주가 앵커(발표일 종가·현재가·52주 범위) |
...
- A가 준 주가 앵커로 메인이 `PRICE_DATA`(30영업일)와 주가 카드를 구성한다.
...
| `PRICE_DATA` | **30 영업일** 종가 | `{d:'M/D',p}` · 발표일 항목에 `earnings:true` |
| `EARN_IDX` | `PRICE_DATA`에서 발표일 0-based 인덱스 | |
| `CURR` | 현재 주가(숫자) | |
| `TP_BARS` | 목표가 바(현재가 제외) | `{label,tp}` · Y축 자동 계산 |
...
### 주가 (`PRICE_DATA`, `TP_BARS`, `CURR`, `EARN_IDX`)
1. **30 영업일** 구성(발표일 전후 포함, 부족하면 이전 날짜로). 확인된 기준점(발표일·익일 공식 종가) 우선, 중간 추정은 `meta.price_disc`에 추정 기준 명기.
2. `TP_BARS`는 증권사(또는 그룹)별 목표가. `meta.tp_low/tp_avg/tp_high/tp_upside`와 정합.

### 애널리스트 (`ANALYSTS`, `CONS`)
최소 3개 이상 증권사 확인. `CONS`는 전체 커버리지 집계(리스트에 안 실린 곳 포함 가능), `meta.coverage_note`·`cons_*_pct`와 정합. 불확실하면 note에 "(추정)".
