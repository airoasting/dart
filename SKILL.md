...
|---|---|---|
| **A. 웹 리서치** | 기업명·종목코드·분기·발표일 | `NEWS`(6~8), `ANALYSTS`(≥3), `CONS`, 목표가 범위(`meta.tp_*`) — **주가는 제외**(price.py로) |
...
| 2026-07-04 | 섹션 라벨 왼쪽 코랄 세로 띠(`.sec-label::before`) 제거 + 좌측 들여쓰기 정리. |
| 2026-07-04 | 기준일(`meta.base_date`)을 빌더가 항상 실행일(오늘)로 자동 스탬프하도록 변경(전일 종가 날짜와 분리). 주가는 전일 종가, 기준일은 리포트 생성일. |
| 2026-07-04 | **주가는 전일 종가로 고정.** 웹 검색 현재가가 출처마다 달라 부정확(카카오 실측 대비 10% 오차)해, `assets/price.py`(Naver 금융 KRX 일봉, requests만 필요)로 전일 종가·등락·52주 범위를 결정론적으로 가져오도록 변경. 에이전트 A 웹 리서치에서 주가 제외. `CURR`·`meta.price/base_date/price_chg/w52_range`가 여기서 나온다. |
